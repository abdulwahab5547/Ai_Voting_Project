...
from core.db import fetch_all, fetch_one, is_voting_open, log_event
...
def login():
    ...
    if not is_voting_open():
        esp32.send("VOTE_CLOSED")
        return jsonify(
            status="closed",
            message="Voting has not started yet. Please wait for the admin.",
        )
    ...
    if voter_id is None:
        ...
        esp32.send("LOGIN_FAIL", reason="No match")
    ...
    if voter["has_voted"]:
        ...
        esp32.send("ALREADY_VOTED", voter_id=voter["id"], name=voter["name"])
        return jsonify(
    ...
    esp32.send("LOGIN_OK", voter_id=voter["id"], name=voter["name"])
    return jsonify(
        status="ok",
